...
// These are now accessible by all functions in the file
...
$(document).ready(function () {
    // 1. Initialize State/UI components
    refreshColorMap();
    updateSidebar();
...
const refreshColorMap = () => {
    ...
    currentData.forEach(period => {
        period.forEach(cat => {
            // Check both naming conventions (color_id vs color_ID)
...
const updateSidebar = () => {
    ...
    // We assume the first period contains the full list of unique categories
    const uniqueCats = currentData[0];

    uniqueCats.forEach(cat => {
        const color = colorMap[cat.category_Name] || '#bdc3c7'; // Fallback to grey
...
const initPerformanceListeners = () => {
...
const initTabListeners = () => {
    $('button[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        // Affordability: Re-render budget cards
        if (targetId === '#affordabilityTabPane') {
            renderBudgetCards(selectedIndex);
        }

        // Goals: Update the main charts
        if (targetId === '#goalsTabPane') {
            renderCharts();
        }

        // Performance: Refresh performance data
...
const initAnalysisControls = () => {
    // Mode Buttons (Annual, Monthly, Forecast, Retirement)
    ...
    // Sub/Super Category Level Buttons
...
const initGlobalAjaxEvents = () => {
    // Re-render charts when category checkboxes are toggled
    $(document).on('change', '.cat-check', renderCharts);

    // Fetch new data from server when global filters change
    $('#openingBalance, #inputYear, #bankAccountSelect, #monthsBack, #monthsForward')
        .on('change', fetchAnalysisData);
};

const fetchAnalysisData = () => {
    // Gather all current UI parameters
    const params = {
        mode: mode,
        level: level,
        year: $('#inputYear').val(),
    ...
    $.get("AnalysisAJAX", params, (res) => {
        currentData = res;

        // Cascade updates to UI components
        refreshColorMap();
        updateSidebar();
        renderCharts();
    });
...
function renderBudgetCards(forecastIndex) {
...
function updateSecondaryViews(index, overrideData = null) {
...
function updateFromSimulation(simData, label) {
